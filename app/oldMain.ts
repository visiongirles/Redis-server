...
const server: net.Server = net.createServer((connection: net.Socket) => {
  ...
  connection.on('data', (data: Buffer) => {
    ...
    // parseCommand = data=> if(data[0] == '*')
    //   { let indexOfRN = findRN(data); if(!indexOfRN) return;
    //     let size = subbuf(data, 1, indexOfRN);
    //     for(let i; i< size;i++){...} }

    //  idxOfEnd = data.indexOf('\r\n');
    //  numberOfArguments = data.subarray(1, idxOfEnd)
    ...
    // начала запроса с *
    // конец запроса - ориентируемся на цифру после *

    // пока не получили весь запрос и пока есть коннекшн храним полученные данные в буфере
