export function handleCommand(data: Buffer | string, response: string) {
  //   const escapeSymbols: string = '\r\n';
  if (data.length < 2)
    throw Error("Request isn't correct. No enough arguments in type of data");

  if (data[0] !== '*') {
    throw Error('first symbol should be asterics');
  }
}
function parseString(argument: string) {
  // \$4\r\nPING\r\n
...

{
  ...
  //TODO: предусмотреть вариант, когда запрос приходит не целиком.
  //  т.е. надо сохранить, что получили  и дождаться продолжения запроса
  ...
  for (index; index <= numberOfArguments * 2; index++) {
    ...
    if (request[index].length < sizeOfData) {
      throw Error(`Request isn't correct. Not enough data in line # ${index}`);
    }
